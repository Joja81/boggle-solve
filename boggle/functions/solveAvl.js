export const solveAvl = (avl, board) => {
  ...
  foundWords.sort(function (first, second) {
  ...
  for (let xChange = -1; xChange < 2; xChange++) {
    for (let yChange = -1; yChange < 2; yChange++) {
      ...
      if (validLocation(newPlace, board.length)) {
        if (
          visitedLocation(used, newPlace) &&
          board[newPlace[0]][newPlace[1]] in avl
        ) {
          let newUsed = used.slice();
          newUsed.push(newPlace);
          exploreAvl(
            newUsed,
            avl[board[newPlace[0]][newPlace[1]]],
            board,
            foundWords
          );
        }
...
function calculateWord(used, board, foundWords) {
  ...
  if (word.length >= 3) {
    ...
    if (!wordFound(foundWords, word)) {
      foundWords.push({ word: word, used: used });
    }
  }
}
...
function wordFound(foundWords, word){
    for (const currWord of foundWords){
